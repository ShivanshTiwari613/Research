...
def generate_research_paper(research_prompt):
    """
    For a given research prompt, generate a complete research paper by processing multiple sections.
    """
    sections = {
...
def generate_endpoint():
    ...
    Returns a text file containing the generated research paper.
    """
    ...
    research_paper_content = generate_research_paper(research_prompt)

    # Create an in-memory file for the response.
    file_io = BytesIO()
    file_io.write(research_paper_content.encode('utf-8'))
    file_io.seek(0)
    return send_file(file_io,
                     as_attachment=True,
                     download_name="research_paper.txt",
                     mimetype="text/plain")
